				    ...
				    GObjectConstructParam *params)
{
  ...
  column = ctk_tree_view_column_new_with_attributes ("databases",
		  				     renderer,
...
gdict_database_chooser_init (GdictDatabaseChooser *chooser)
{
  ...
  priv->store = ctk_list_store_new (DB_N_COLUMNS,
		                    G_TYPE_INT,    /* db_type */
                  ...
                  gpointer      user_data)
{
  ...
  ctk_tree_model_get (model, iter, DB_COLUMN_NAME, &db_name, -1);
  if (!db_name)
    return FALSE;
...
gdict_database_chooser_get_current_database (GdictDatabaseChooser *chooser)
{
  ...
  ctk_tree_model_get (model, &iter, DB_COLUMN_NAME, &retval, -1);
                                   ...
                                   const gchar          *button_text)
{
  ...
  button = ctk_button_new_from_stock (button_text);
